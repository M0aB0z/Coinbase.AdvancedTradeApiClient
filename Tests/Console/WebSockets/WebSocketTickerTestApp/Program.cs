...
try
{
    ...
    await webSocketManager.SubscribeAsync(["BTC-USDC"], ChannelType.Ticker, CancellationToken.None);
...
async Task CleanupAsync(WebSocketManager? webSocketManager)
{
    ...
    await webSocketManager!.UnsubscribeAsync(["BTC-USDC"], ChannelType.Ticker, CancellationToken.None);
