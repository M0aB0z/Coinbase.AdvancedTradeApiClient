...
try
{
    ...
    await webSocketManager.SubscribeAsync([], ChannelType.User, CancellationToken.None);
...
async Task CleanupAsync(WebSocketManager? webSocketManager)
{
    ...
    await webSocketManager!.UnsubscribeAsync([], ChannelType.User, CancellationToken.None);
